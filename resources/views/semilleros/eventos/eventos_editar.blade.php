<x-guest-layout>
    ...
    <x-authentication-card>
        ...
        <form method="POST" action="{{ isset($evento) ? route('actualizar_evento', [$evento->cod_evento]) : route('registrar_evento') }}" enctype="multipart/form-data">
            @isset($evento)
                @method('PUT')
            @endisset
            @csrf
            ...
            <div class="flex items-center justify-end mt-4">
                <button type="submit" class="btn-register">
                    {{ isset($evento) ? __('Guardar cambios') : __('Registrar') }}
                </button>
            </div>
        </form>
    </x-authentication-card>
